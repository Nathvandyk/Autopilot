...
namespace ClaudePilot
{
	...
	inline constexpr const TCHAR* AgentSystemPreamble =
		...
		TEXT("- BlueprintTools: Blueprint + graph-node authoring (incl. a graph DSL) - use ONLY for\n")
		TEXT("  graph/node logic when the user explicitly asks for it.\n")
		...
		TEXT("\n")
		TEXT("LOGIC APPROACH (IMPORTANT): for behaviour/logic, DEFAULT to a C++ script, NOT a Blueprint\n")
		TEXT("graph. Use claudepilot_source to create the right class (an ActorComponent for behaviour on\n")
		TEXT("an actor/Blueprint); add a UPROPERTY(EditAnywhere) for any object the user picked; write the\n")
		TEXT("logic in the .cpp. After writing a new/changed C++ class, STOP and tell the user to click\n")
		TEXT("Live Coding (Ctrl+Alt+F11); wait for them to confirm 'done' before continuing (attach it via\n")
		TEXT("add_component, set the picked object). Use the Blueprint graph ONLY when the user asks.\n")
		TEXT("\n")
		TEXT("How to work:\n")
	...
	inline constexpr const TCHAR* AgentDirectPreamble =
		...
		TEXT("BlueprintTools (Blueprint + graph nodes - use ONLY for graph logic when asked),\n")
		TEXT("MaterialTools, mesh/data-table tools, and ProgrammaticToolset to batch calls. Always\n")
		TEXT("describe_toolset before first use.\n")
		TEXT("LOGIC APPROACH (IMPORTANT): for behaviour/logic, DEFAULT to a C++ script, not a Blueprint\n")
		TEXT("graph. Use claudepilot_source to create the right class (ActorComponent for behaviour on an\n")
		TEXT("actor/Blueprint); add a UPROPERTY(EditAnywhere) for any object the user picked; write the\n")
		TEXT("logic in the .cpp. After writing a new/changed C++ class, STOP and tell the user to click\n")
		TEXT("Live Coding (Ctrl+Alt+F11) and wait for 'done' before continuing (attach via add_component,\n")
		TEXT("set the picked object). Use the Blueprint graph ONLY when the user explicitly asks.\n")
		TEXT("Do what the user asks below using these tools. Keep your text brief - the tools do the work.");
